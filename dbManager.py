...
class dbManager():
    ...
    def get_all_data_from_db(self, iter_cur :bool = False, **kwargs):
        try:
    ...
    def delete_data_from_db(self, **kwargs):
        '''
        Отправляет запрос в БД для удаления записи по ключевым значениям
        '''
        ...
        sql_request :Cursor = self.__cur.execute(f"DELETE FROM {self.table_name} WHERE {db_request}")
        self.__connection.commit()

        return

    def update_data_from_db(self, **kwargs):
        '''
        Отправляет запрос в БД для удаления записи по ключевым значениям
        '''
        ...
        sql_request :Cursor = self.__cur.execute(f"UPDATE {self.table_name} SET {db_request}")
        self.__connection.commit()
    ...
    def MemsDB(ctx):  #, table_fields={"name" : "TEXT", "url" : "TEXT", "id_user" : "INT"}):
        ...
        return ctx(table_name = table_name,\
            table_fields = {"name" : "TEXT", "age" : "INT", "id" : "TEXT"})
